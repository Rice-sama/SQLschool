...
public class JsonManager {
	...
	public void createJSON(String nomFichier) {
		...
		try (FileWriter file = new FileWriter(nomFichier)) {
			...
			JOptionPane.showMessageDialog(null, "Le fichier sp�cifi� est inutilisable.", "Attention",
	...
	public void readFileQuestion(String nomFichier, Boolean add) {
		...
		try (FileReader reader = new FileReader(new File(nomFichier))) {
			...
			System.out.println("Le fichier sp�cifi� est introuvable.j ");
			JOptionPane.showMessageDialog(null, "Le fichier sp�cifi� est introuvable.", "Attention",
					JOptionPane.WARNING_MESSAGE);
		}
	...
	public boolean addQuestion(String database, String subject, String titre, String contenu, String bonne_reponse, ArrayList<Test> testList) {
		...
		if (listQuestions.contains(q)) {
			System.out.println("Cette question existe d�ja.");
			return false;
		}
		listQuestions.add(q);
		return true;
